<?php
    ...
    $sql = 'select * from reviews where movieId='. $_GET['movieId'] .' order by createdDate desc';
    $array_reviews = array();
    ...
    try{
        if ($result = mysqli_query($conn, $sql)) {
            ...
            $error->code = 400;
            $error->description = "Get Reviews By Movie: ". mysqli_error($conn);
            $error->success = false;
        ...
        $error = new CustomError;
        $error->code = 400;
        $error->description = "Get Reviews By Movie: ". $e->getMessage();
        $error->success = false;
        echo json_encode($error);
    }
?>